...
class StrategicDataPoisoner:
    ...
    def select_samples_for_poisoning(self, X: np.ndarray, y: np.ndarray,
...
def load_datasets():
    ...
    return X_train, y_train, X_val, y_val, X_test, y_test, scaler, label_encoder, feature_names
...
def test_multiple_poison_rates():
    ...
    X_train, y_train, X_val, y_val, X_test, y_test, scaler, label_encoder, feature_names = load_datasets()
    ...
    for poison_rate in POISON_RATES_TO_TEST:
        ...
        model, label_to_idx, idx_to_label, input_dim, num_classes = load_pretrained_model(
        ...
        for step in range(1, NUM_STEPS + 1):
            ...
            for epoch in range(2):
                train_epoch(model, X_tr_poisoned, y_tr_poisoned, 
                          optimizer, criterion, device, batch_size=256)
            ...
            if step % 2 == 0 or step == 1:
                asr_str = " | ".join([f"{cls}={asr_current[cls]:.4f}" for cls in TARGET_CLASSES])
...
def plot_results(all_results: Dict):
    ...
    colors_class = {'DDoS': '#e74c3c', 'Injection': '#3498db', 'Normal': '#2ecc71', 'Probe': '#f39c12', 'Scanning': '#9b59b6'}
    linestyles = ['-', '--', ':', '-.']
    class_linestyle = {
    ...
    for pr_idx, pr in enumerate(poison_rates):
        ...
        for target_cls in TARGET_CLASSES:
            ...
            ax.plot(range(len(asr_per_step)), asr_per_step, 
                   marker='o', label=f'{target_cls} ({pr:.1%})', 
                   color=poison_color_map[pr],
                   linewidth=2, linestyle=class_linestyle[target_cls], alpha=0.85)
    
    ax.set_xlabel('Poisoning Step', fontsize=12, fontweight='bold')
    ax.set_ylabel('ASR', fontsize=12, fontweight='bold')
    ax.set_title('ASR Evolution per Class for Different Poison Rates', fontsize=13, fontweight='bold')
    ...
    for i, cls in enumerate(class_list):
        ...
        ax.bar(x + offset, accs, width, label=cls, 
              color=colors_class_bar[i % len(colors_class_bar)], 
              alpha=0.8, edgecolor='black', linewidth=0.5)
    
    ax.set_xlabel('Poison Rate', fontsize=12, fontweight='bold')
    ax.set_ylabel('Accuracy', fontsize=12, fontweight='bold')
    ax.set_title('Final Accuracy per Class for Different Poison Rates', fontsize=13, fontweight='bold')
    ax.set_xticks(x)
    ...
    ax.set_title('Summary Table: ASR and Poisoning Statistics', 
...
def print_summary(all_results: Dict):
    ...
    for pr in poison_rates:
        ...
        print(f"{pr:>6.1%}{'':<8} {results['baseline_asr']:>6.4f}{'':<8} {results['final_asr']:>6.4f}{'':<8} {results['asr_improvement']:>+6.4f}{'':<8} {results['poisoned_samples']:>8}")
...
def main():
    ...
    for pr, res in all_results.items():
        results_to_save[f'{pr:.1%}'] = {
            ...
            'time': float(res['time']),
            'asr_per_class': {str(k): float(v) for k, v in res['final_asr_dict'].items()}
        }
