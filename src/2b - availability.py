...
def poison_data_inplace(
    X, y, step, poison_fraction, noise_level, alpha=0.25, mask=None, seed=42
):
...
def sample_memory(data_by_class, classes, memory_size):
    ...
    for cls in classes:
        ...
        y_list.append(np.full(n, CLASS_TO_IDX[cls], dtype=np.int64))
...
def train_offline_model(
    X_tr, y_tr, X_v, y_v, input_dim, num_classes, lr=1e-3, epochs=50, patience=5
):
...
def run_experiment_fast_pretrained(
    ...
    for step in range(1, NUM_CLASSES + 1):
        ...
        if poisoned and step > 1:
            X_tr = X_tr.copy()
            poison_data_inplace(X_tr, y_tr, step, poison_fraction, noise_level)
        ...
        for cls_idx, cls_name in enumerate(CLASS_ORDER[:step]):
            mask = y_te == cls_idx
            acc = evaluate(model, X_te[mask], y_te[mask]) if mask.any() else np.nan
...
def main():
    ...
    train_by_class, val_by_class, test_by_class = prepare_data_once(
        X_train, y_train, X_val, y_val, X_test, y_test, label_encoder, LABEL_MAP
    )
