...
def load_cicids_dataset(
    data_path="datasets/CICIDS-2017/", label_column="Label", encoding="latin1"
):

    print(f"\n\nLOADING CICIDS DATASET AT PATH {data_path}...")
    csv_files = list(
        Path(data_path).rglob("*.csv")
    )  # Recursively search for csv files in data_path
    if not csv_files:
    ...
    combined_dataframes = pd.concat(
        dataframes, ignore_index=True
    )  # Merges retrieved dataframes
    ...
    combined_dataframes[label_column] = (
        combined_dataframes[label_column]
...
def load_toniot_dataset(
    data_path="datasets/ToN-IoT/", label_column="type", encoding="utf-8"
):
    # LOADING
...
def features_cleanup(df, label_column, corr_threshold=0.95, memory_efficient=True):
    ...
    if memory_efficient and len(keep_cols) > 100:
        ...
        for i, col1 in enumerate(features):
            ...
            remaining_features = [
                c for c in features[i + 1 :] if c not in features_to_drop
            ]
...
if __name__ == "__main__":
    ...
    cicids_df, cicids_label_col = load_cicids_dataset(
        "../../data/datasets/CICIDS-2017/", "Label", "latin1"
    )
    ...
    toniot_df, toniot_label_col = load_toniot_dataset(
        "../../data/datasets/ToN-IoT/cicflowmeter_cicids_no_unknown", "type"
    )
    ...
    unified_df, unified_label_col = unify_datasets(
        cicids_df_clean, cicids_label_col, toniot_df_clean, toniot_label_col
    )
